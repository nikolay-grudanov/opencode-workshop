...
- [ ] 5.6 Commit `feat: sub-agent drill-down and timeline bands` (body: `Refs F-003. Multi-level drill-down with breadcrumb chain; nested sub-agents visible in focused view; FlameTimeline gold bands/bars with click-to-dive; SpanTree in-tree dive-in.`) — only on Kolya's explicit OK
